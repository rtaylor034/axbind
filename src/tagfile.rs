...
impl TagFile<'_> {
    pub fn generate_from<'t>(table: &TableHandle<'t>) -> Result<TagFile<'t>, ConfigError> {
        ...
        let files: Vec<&Path> = get_array_strings(table, "files")?.into_iter().map(|file| Path::new(file)).collect();
        ...
        let functions = get_array_strings(&scheme_table, "functions").optional()?.unwrap_or(vec![]);
